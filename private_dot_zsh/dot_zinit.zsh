...
zirs for \
    ...
    id-as'__theme_Enki-Tokyo-Night' as'null' \
        atclone'
            command mv -fv __theme_Enki-Tokyo-Night Enki-Tokyo-Night
            (){
                local themedir="$(bat --config-dir)/themes"
                [[ ! -d "$themedir" ]] && mkdir -p "$themedir"
                command cp -sfv "$PWD/Enki-Tokyo-Night" "$themedir/Enki-Tokyo-Night.tmTheme"
            } && command bat cache --build' atpull'%atclone' \
...
zirs for sbin'bin/dog' from'gh-r' bpick'*x86_64-unknown-linux-gnu.zip' \
    atclone'command mv -vf **/dog.zsh _dog; mansym $PWD/**/dog.1' atpull'%atclone' @ogham/dog
...
zirs for sbin'tldr' from'gh-r' bpick'*linux-x86_64-musl' bpick'*_zsh' \
    mv'tldr*musl -> tldr' \
    atclone'./tldr -u > /dev/null; command mv -vf *_zsh _tldr' atpull'%atclone' @dbrgn/tealdeer
...
#         #     [[ $EDITOR = vim && $(vim --version |command grep "\+clientserver") ]] &&
